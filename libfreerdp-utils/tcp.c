...
int freerdp_tcp_connect(freerdp* instance, const char* hostname, int port)
{
	...
	if (status != 0)
	{
		freerdp_log(instance, "tcp_connect: getaddrinfo (%s)\n", gai_strerror(status));
		return -1;
	}
...
int freerdp_tcp_read(freerdp* instance, int sockfd, uint8* data, int length)
{
	int status;
...
int freerdp_tcp_write(freerdp* instance, int sockfd, uint8* data, int length)
{
	int status;
