/*
   ...
   Clipboard Virtual Channel - iOS implementation

   Copyright 2011 Raf Cabezas
...
typedef int Atom;
typedef void XEvent;
typedef void XSelectionRequestEvent;
...
clipboard_get_requested_targets(struct clipboard_data * cdata)
{
	Atom atom;
	int format;
	unsigned long len = 0, bytes_left;
...
clipboard_get_requested_data(struct clipboard_data * cdata, Atom target)
{
	Atom type = 0;
	int format, result;
	//unsigned long len, bytes_left = 0, dummy;
...
clipboard_new(cliprdrPlugin * plugin)
{
	...
	cdata->plugin = plugin;
	cdata->term_event = wait_obj_new("freerdpcliprdrremoterterm");
